...
Public Class CustomerTransactionHistory
    ...
    Private Sub BtnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
        'Take the date from the DateTimePicker and the cafe name from the ComboBox
        Dim datePicked As Date = DateTimePicker1.Value.Date
        'if no cafe is selected, display all transactions
        If cboCafe.SelectedIndex = -1 Then
            transactions = GetTransactions(DateTimePicker1.Value, cboCafe.SelectedItem)
            DisplayTransactions(transactions)
        Else
            Dim cafeName As String = cboCafe.SelectedItem.ToString()
            ' If "All Cafe" is selected, retrieve all transactions for the user
            If cafeName = "All Cafe" Then
                transactions = GetTransactions()
                DisplayTransactions(transactions)
            Else
                'Retrieve transactions for the specific user ID, date, and cafe Name
                transactions = GetTransactions(DateTimePicker1.Value, cboCafe.SelectedItem)
                DisplayTransactions(transactions)
            End If
        End If
